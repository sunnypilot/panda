  ...
  if (valid) {
    ...
    if (!(alternative_experience & ALT_EXP_DISABLE_STOCK_AEB)) {
      if ((bus == 2) && (addr == 0x1FA)) {
        ...
        if (honda_clarity_brake_msg) honda_stock_brake = (GET_BYTE(to_push, 6) << 2) + ((GET_BYTE(to_push, 7) >> 6) & 0x3U);
...
static int honda_tx_hook(CANPacket_t *to_send) {
  ...
  if ((addr == 0x1FA) && (bus == bus_pt)) {
    ...
    if (honda_clarity_brake_msg) honda_brake = (GET_BYTE(to_send, 6) << 2) + ((GET_BYTE(to_send, 7) >> 6) & 0x3U);
